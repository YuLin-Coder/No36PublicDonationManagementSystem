...
<head>
	...
	<link rel="stylesheet" href="${ctxStatic}/layui/lay2/modules/skin/layer.css" media="screen" type="text/css">
	...
	<script type="text/javascript">
		...
		function frozenTraw(userId,delFlag){
			$.ajax({
                cache: true,
                type: "POST",
                url: "${ctx}/sys/user/frozenTraw",
                data: {
                	"userId" : userId,
                	"delFlag" : delFlag
                },
                ...
                success: function (dataMap) {
                    if (dataMap.status == 0) {
                    	layer.msg(dataMap.result, {icon: 1});
                    	setTimeout(function(){
                    		window.location.reload();
                     	},800);
                    	
                    } else {
                    	layer.msg(dataMap.result, {icon: 2});
                    }
                }
            });
		}
			...
			<tr>
				...
				<shiro:hasPermission name="sys:user:edit"><td>
					...
					<c:if test="${user.delFlag == '2'}">
						<button class="layui-btn layui-btn-mini" type="button" onclick="frozenTraw('${user.id}', '0')">
							<i class="layui-icon">&#xe64d;</i>&nbsp;解冻
						</button>
					</c:if>
					<c:if test="${user.delFlag == '0'}">
						<button class="layui-btn layui-btn-warm layui-btn-mini" type="button" onclick="frozenTraw('${user.id}', '2')">
